...
# A bare filename, one path segment, no traversal. Job files (candidate.py,
# test_candidate.py, conftest.py, ...) are always simple names generated by trusted code,
# but write_file's `name` argument comes from a caller rather than being a hardcoded
# constant, so it is validated rather than trusted blindly.
...
class SandboxWorkspace:
    ...
    def write_file(self, name: str, content: str) -> Path:
        """Write ``content`` verbatim to ``name`` inside the workspace.

        The source is written to a file and never interpolated into a command (spec 05
        sections 42, 43), which is what removes shell quoting and injection from the
        picture entirely. ``name`` must be a bare filename — no directory separators, no
        ``..`` — since it can come from a caller (e.g. Stage 6's evaluation job, which
        writes ``candidate.py``, ``test_candidate.py``, and ``conftest.py`` into one
        workspace) rather than always being the fixed ``candidate.py`` constant.
        """
        if not isinstance(content, str):
            raise WorkspaceError(f"{name}: content must be a string")
        if not _SAFE_FILENAME_RE.match(name) or "/" in name or "\\" in name:
            raise WorkspaceError(f"{name!r} is not a safe bare filename")
    ...
    def write_candidate(self, code: str) -> Path:
        """Write ``code`` verbatim to ``candidate.py``. A thin wrapper over
        :meth:`write_file` kept for Stage 5 callers and tests.
        """
        return self.write_file(CANDIDATE_FILENAME, code)
